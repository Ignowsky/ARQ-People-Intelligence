...
def carregar_dim_calendario(engine, schema):
    ...
    BEGIN
        ...
        SELECT
            ...
            EXTRACT(DOW FROM d), to_char(d, 'TMDay'), to_char(d, 'TMMonth'),
            to_char(d, 'TMMon'), to_char(d, 'YYYY-MM'), EXTRACT(DOY FROM d), EXTRACT(WEEK FROM d)
        FROM generate_series(data_inicio, data_fim, '1 day'::interval) d
        ON CONFLICT (data) DO NOTHING; 
    END $$;
    """)
    with engine.begin() as conn: conn.execute(sql)
    print("Dimensão Calendário verificada.")
...
def carregar_fatos_folha(df_consol, df_detalhe, engine, schema):
    ...
    if not df_consol.empty:
        ...
        # Como o DF já foi tratado no Transform, podemos confiar no None
        ...
        CREATE TABLE IF NOT EXISTS "{schema}"."dim_colaboradores_base" (
            colaborador_sk SERIAL PRIMARY KEY, nome_colaborador VARCHAR(255) NOT NULL, cpf VARCHAR(20) UNIQUE NOT NULL,
            data_admissao_csv DATE, data_demissao_csv DATE, situacao_csv VARCHAR(100), departamento_csv VARCHAR(255), cargo_csv VARCHAR(255)
        );
        ...
        -- INSERT SIMPLIFICADO (Tipos já tratados no Python/Staging)
        INSERT INTO "{schema}"."dim_colaboradores_base" (
            nome_colaborador, cpf, data_admissao_csv, data_demissao_csv, situacao_csv, departamento_csv, cargo_csv
        )
        SELECT DISTINCT ON (cpf)
            nome_colaborador, cpf,
            CAST(data_admissao_csv AS DATE), CAST(data_demissao_csv AS DATE), 
    ...
    if not df_consol.empty:
        comps = tuple(df_consol['competencia'].dropna().unique())
        if comps:
            df_consol.to_sql("stg_folha_consol", engine, schema=schema, if_exists='replace', index=False, dtype=SCHEMA_TOTAIS)
            
            sql_consol = f"""
                CREATE TABLE IF NOT EXISTS "{schema}"."fato_folha_consolidada" (
                    fato_folha_id SERIAL PRIMARY KEY, colaborador_sk INTEGER, competencia DATE,
                ...
                SELECT
                    COALESCE(base.colaborador_sk, 0), stg.competencia, 
                    stg.nome_funcionario, stg.departamento, stg.cargo, stg.cpf, stg.situacao, stg.tipo_calculo,
            ...
            """
            with engine.begin() as conn: conn.execute(text(sql_consol), {'comps': comps})
    ...
    if not df_detalhe.empty:
        ...
        if comps_det:
            df_detalhe.to_sql("stg_folha_detalhe", engine, schema=schema, if_exists='replace', index=False, dtype=SCHEMA_RUBRICAS)
            
            sql_detalhe = f"""
                CREATE TABLE IF NOT EXISTS "{schema}"."fato_folha_detalhada" (
                    fato_rubrica_id SERIAL PRIMARY KEY, colaborador_sk INTEGER, competencia DATE,
                ...
                LEFT JOIN "{schema}"."dim_colaboradores_base" base ON stg.cpf = base.cpf;
            """
            with engine.begin() as conn: conn.execute(text(sql_detalhe), {'comps': comps_det})
            print("Fato Detalhada carregada.")

# --------------------------------------------------------------------------------
# CARGA API (COLABORADORES + BENEFÍCIOS)
# --------------------------------------------------------------------------------
def carregar_dados_api(df_staging, df_beneficios, engine, schema):
    ...
    # Garante que os DataFrames tenham as colunas esperadas como string
    # (Para evitar problemas com None virando float NaN)
    df_staging['cpf'] = df_staging['cpf'].astype(str).replace('nan', None)
    df_beneficios['colaborador_id_solides'] = df_beneficios['colaborador_id_solides'].astype(float).astype('Int64') # Int64 aceita null

    # Helper SQL (Mantido para API)
    def to_num(col):
    ...
    try:
        # Carrega Staging (Substitui tabela existente)
        ...
        -- Inserção na Base a partir da Staging da API
        ...
        CREATE TABLE IF NOT EXISTS "{schema}".{NOME_TABELA_RICA} (
            ...
            telefone_pessoal VARCHAR(50), celular VARCHAR(50), nacionalidade VARCHAR(100), 
            nivel_educacional VARCHAR(100), nome_mae VARCHAR(255), nome_pai VARCHAR(255),
            logradouro VARCHAR(255), numero_endereco VARCHAR(50), complemento_endereco VARCHAR(100),
            bairro VARCHAR(100), cidade VARCHAR(100), estado VARCHAR(50), cep VARCHAR(20),
            matricula VARCHAR(50), estado_civil VARCHAR(50), salario_api NUMERIC(12, 2),
            turno_trabalho VARCHAR(100), tipo_contrato VARCHAR(100), curso_formacao VARCHAR(255),
            nivel_hierarquico VARCHAR(100), nome_lider_imediato VARCHAR(255), etnia VARCHAR(50),
            unidade_nome VARCHAR(255), data_ultima_atualizacao TIMESTAMP DEFAULT current_timestamp,
        ...
        ALTER TABLE "{schema}".{NOME_TABELA_RICA}
            ...
            ADD COLUMN IF NOT EXISTS data_ultima_atualizacao_api DATE, ADD COLUMN IF NOT EXISTS cargo_id_solides INTEGER,
            ADD COLUMN IF NOT EXISTS departamento_id_solides INTEGER, ADD COLUMN IF NOT EXISTS saudacao VARCHAR(50),
            ADD COLUMN IF NOT EXISTS tipo_necessidade_especial VARCHAR(100), ADD COLUMN IF NOT EXISTS local_nascimento VARCHAR(100),
            ADD COLUMN IF NOT EXISTS pcd BOOLEAN, ADD COLUMN IF NOT EXISTS telefone_emergencia VARCHAR(50),
            ADD COLUMN IF NOT EXISTS email_pessoal VARCHAR(255), ADD COLUMN IF NOT EXISTS email_corporativo_sec VARCHAR(255),
            ADD COLUMN IF NOT EXISTS rg VARCHAR(50), ADD COLUMN IF NOT EXISTS data_emissao_rg DATE,
            ADD COLUMN IF NOT EXISTS orgao_emissor_rg VARCHAR(50), ADD COLUMN IF NOT EXISTS titulo_eleitor VARCHAR(50),
            ADD COLUMN IF NOT EXISTS zona_eleitoral VARCHAR(50), ADD COLUMN IF NOT EXISTS secao_eleitoral VARCHAR(50),
            ADD COLUMN IF NOT EXISTS ctps_numero VARCHAR(50), ADD COLUMN IF NOT EXISTS ctps_serie VARCHAR(50),
            ADD COLUMN IF NOT EXISTS pis VARCHAR(50), ADD COLUMN IF NOT EXISTS motivo_demissao_api VARCHAR(255),
            ADD COLUMN IF NOT EXISTS data_contrato DATE, ADD COLUMN IF NOT EXISTS duracao_contrato VARCHAR(100),
            ADD COLUMN IF NOT EXISTS data_expiracao_contrato DATE, ADD COLUMN IF NOT EXISTS periodo_experiencia_dias INTEGER,
            ADD COLUMN IF NOT EXISTS forma_demissao VARCHAR(100), ADD COLUMN IF NOT EXISTS decisao_demissao VARCHAR(100),
        ...
        ON CONFLICT (colaborador_id_solides) DO UPDATE SET
            cpf = EXCLUDED.cpf, nome_completo = EXCLUDED.nome_completo,
            data_admissao = EXCLUDED.data_admissao, data_demissao = EXCLUDED.data_demissao,
            ativo = EXCLUDED.ativo, departamento_nome_api = EXCLUDED.departamento_nome_api,
            salario_api = EXCLUDED.salario_api, data_ultima_atualizacao = current_timestamp;
        ...
        -- AQUI: Garantindo que o JOIN use as tabelas corretas
    ...
    except Exception as e: print(f"Erro Carga API: {e}")
...
def processar_status_transferidos(engine, schema):
    ...
    except Exception as e: print(f"Erro Transferidos: {e}")